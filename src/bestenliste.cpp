...
/*Methode, um die Namen in der Bestenliste neu zuordnen zu können.
Sie bekommt den aktuellen Namen in der Bestenliste und den zu überprüfenden Namen übergeben.*/
void Bestenliste::set_name(char* name_alt, char* name)
{
    for (int i=0; i<15; i++)
    {
       name[i]=name_alt[i];
...
/*Methode zum Soertieren und Befüllen der Bestenliste. Es wird der zu überprüfende Name und die Punktzahl übergeben.*/
void Bestenliste::fuellenBestenliste(int punktzahl, char* name)
{
    if (punktzahl>platz1)       //Prüfen, ob die zu überprüfende Punktzahl höher als die des Platz 1 der Bestenliste ist
    {
